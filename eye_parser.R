...
zero_crossing <- function(x, y, thr = 0, find_rising_edge = TRUE, interp = FALSE) {
  if (find_rising_edge) {
...
zero_crossing_wfm <- function(ref_i, wfm_t, wfm_v, find_rising_edge = TRUE, interp = TRUE) {
...
f1_load_raw_wfm <- function() {
  format_message(str_c("f1: load raw wfm ", str_c(p_in, wfm_file)))
...
f1_load_raw_ref <- function() {
  format_message(str_c("f1: load raw ref ", str_c(p_in, ref_file)))
...
f1_view_raw_ref <- function() {
  p_ref <- ggplot(
    df_ref_raw %>% mutate(`#` = seq_along(df_ref_raw$MK1)),
    aes(`#`, MK1)
  ) +
    geom_step() +
    # geom_point() +
...
f2_upsample <- function() {
  ...
  if ((Fs %% F) > 0) {
    ...
    file_new_wfm <- str_c(p_out, "new_wfm.txt")
    ...
    dfp <- rbind(
      ...
      tibble(S = df_wfm$S[1:(cnt * up)], V = df_wfm$V[1:(cnt * up)], type = "upsampled")
    )
    ...
    ggp <- ggplotly(p)
    print(ggp)
    html_name <- str_c(p_fig, "f2_upsample.html")
...
f3_detect_pattern <- function() {
  ...
  ref_one_cycle <- df_ref_raw$MK1

  c <- ccf(ref_one_cycle, ref_one_cycle, plot = TRUE, lag.max = (length(ref_one_cycle) - 1))
  c_max <- max(c$acf)
  ...
  ref_one_prbs_with_tags <- set_names(pats[1:prbs_length], tags[(prbs_length + 1):(prbs_length * 2)])
  ref_raw_with_tags <- rep(ref_one_prbs_with_tags, ceiling(length(ref_one_cycle) / prbs_length))[1:length(ref_one_cycle)]
  df_ref_raw_with_tags <<- tibble(
    tags = names(ref_raw_with_tags),
    V = ref_raw_with_tags
  )
  file_ref_raw_with_tags <- str_c(p_out, "ref_raw_with_tags.txt")
  ...
  ggp <- ggplotly(p)
  print(ggp)
  html_name <- str_c(p_fig, "f3_detect_pattern.html")
  htmlwidgets::saveWidget(ggp, html_name)
  message("save to ", html_name)
}
...
f4_roll_ref <- function() {
  ...
  c <- ccf(wfm_one_cycle, ref_one_cycle, lag.max = (length(wfm_one_cycle) - 1), plot = FALSE)
  c_max <- max(c$acf)
  c_min <- min(c$acf)
  ...
  v <- rep(ref_one_cycle_rolled, ceiling(length(df_wfm$V) / length(ref_one_cycle_rolled)))[1:length(df_wfm$V)]
  ...
  file_new_ref <- str_c(p_out, "new_ref.txt")
  ...
  dfp <- rbind(
    tibble(`#` = seq_along(wfm_one_cycle), y = wfm_one_cycle * scale, type = "wfm (normalized to ref)"),
  ...
  ggp <- ggplotly(p)
  print(ggp)
  html_name <- str_c(p_fig, "f4_roll_ref.html")
  htmlwidgets::saveWidget(ggp, html_name)
  message("save to ", html_name)
}
f5_find_zero_crossing <- function() {
  format_message("f5: find zero crossing")

  t <- zero_crossing(df_ref$S, df_ref$V, thr = 0, find_rising_edge = TRUE, interp = FALSE)
  df_ref_rise <<- tibble(
    i = t$i,
    S = t$x,
    V = t$y,
  ...
  t <- zero_crossing(df_ref$S, df_ref$V, thr = 0, find_rising_edge = FALSE, interp = FALSE)
  df_ref_fall <<- tibble(
    i = t$i,
    S = t$x,
    V = t$y,
  ...
  t <- zero_crossing_wfm(df_ref_rise$i, df_wfm$S, df_wfm$V, find_rising_edge = TRUE, interp = TRUE)
  df_wfm_rise <<- tibble(
    i = t$i,
    S = t$x,
    V = t$y,
  ...
  t <- zero_crossing_wfm(df_ref_fall$i, df_wfm$S, df_wfm$V, find_rising_edge = FALSE, interp = TRUE)
  df_wfm_fall <<- tibble(
    i = t$i,
    S = t$x,
    V = t$y,
...
f6_gen_tie <- function() {
  format_message("f6: generate tie")
  ...
  ref_clk_rising_edges_t <- (df_ref_rise$i - first_zero_crossing_i) * 1 / Fs + first_zero_cross_wfm_t
  ref_clk_falling_edges_t <- (df_ref_fall$i - first_zero_crossing_i) * 1 / Fs + first_zero_cross_wfm_t
  ...
  ggp <- ggplotly(p)
  print(ggp)
  html_name <- str_c(p_fig, "f6_gen_tie.html")
  htmlwidgets::saveWidget(ggp, html_name)
  message("save to ", html_name)
}

f7_gen_eye_db <- function() {
  format_message("f7: generate eye database")
  eye_samples_per_T <<- OS
  eye_samples_per_Window <<- (eye_window_length_l + eye_window_length_r) * eye_samples_per_T

  temp <- min(df_wfm_rise$i[1], df_wfm_fall$i[1]) - (eye_window_length_l - 0.5) * eye_samples_per_T
  eye_start_i <- ifelse(temp > 0, temp, (temp + ceiling(-temp / eye_samples_per_T) * eye_samples_per_T))

  max_iter <- floor((length(df_wfm$V) - eye_start_i + 1) / eye_samples_per_Window)
  ...
  for (k in 1:max_iter) {
    slice_index <- eye_start_i + (k - 1) * eye_samples_per_T + (1:eye_samples_per_Window - 1)
  ...
  file_eye_db <- str_c(p_out, "eye_db.txt")
  write_tsv(df_eye_db, file_eye_db)
  message("save to ", file_eye_db)

  file_eye_db_i <- str_c(p_out, "eye_db_i.txt")
  write_tsv(df_eye_db_i, file_eye_db_i)
  message("save to ", file_eye_db_i)

  file_eye_db_transition_tags <- str_c(p_out, "eye_db_tags.txt")
...
f8_gen_eye_jitter <- function(tag = NA) {
  ...
  if (!is.null(df_eye_db_transition_tags) && !is.na(tag)) {
    format_message(str_c("f8: eye jitter for tag ", tag))
    ...
    format_message("f8: eye jitter for all transitions")
  ...
  for (j in 1:n_col) {
    ...
    if ((a < b) && (a * b < 0)) { # rising edge
      t <- zero_crossing(x0, y0, thr = thr, find_rising_edge = TRUE, interp = TRUE)
      wi <- t$i
      wx <- t$x
      wy <- t$y
    ...
    if ((a > b) && (a * b < 0)) { # falling edge
      t <- zero_crossing(x0, y0, thr = thr, find_rising_edge = FALSE, interp = TRUE)
      wi <- t$i
      wx <- t$x
      wy <- t$y
      jitter_t0 <- c(jitter_t0, wx)
    ...
    b <- y1[length(y1)]
    if ((a < b) && (a * b < 0)) { # rising edge
      t <- zero_crossing(x1, y1, thr = thr, find_rising_edge = TRUE, interp = TRUE)
      wi <- t$i
      wx <- t$x
      wy <- t$y
    ...
    if ((a > b) && (a * b < 0)) { # falling edge
      t <- zero_crossing(x1, y1, thr = thr, find_rising_edge = FALSE, interp = TRUE)
      wi <- t$i
      wx <- t$x
      wy <- t$y
      jitter_t1 <- c(jitter_t1, wx)
  ...
  file_eye_jitter <- str_c(p_out, "eye_jitter.txt")
  ...
  if (is.na(tag)) {
    ...
    message("eye jitter PkPk ", sprintf("%.3f", eye_jitter_pkpk_pS_for_tag), "pS")
    message("don't update eye width")
    message("don't update eye datarate")
  }
  df_eye_t <<- tibble(pS = (loc_eye_t - mean(jitter_t0)) / (1 / F * 1e12))
  file_eye_t <- str_c(p_out, "eye_t.txt")
  ...
  file_eye_property <- str_c(p_out, "eye_property.txt")
