...
Eye <- R6Class(
  ...
  private = list(
    ...
    zero_crossing = function(x, y, thr = 0, find_rising_edge = T, interp = F) {
      ...
      if (interp) {
        zc_x <- c()
        for (i in zc_i) {
          zc_x <- c(zc_x, private$linear_interp(x[i], y[i], x[i + 1], y[i + 1], thr))
        }
    ...
    zero_crossing_wfm = function(ref_i, wfm_t, wfm_v, find_rising_edge = T, interp = T) {
      ...
      for (i in ref_i) {
        ...
        t <- private$zero_crossing(wfm_t[rng], wfm_v[rng], 0, find_rising_edge, interp)
        wi <- t$i
        wx <- t$x
        wy <- t$y
  ...
  public = list(
    ...
    initialize = function( # p_in = "/Users/v/My Drive/pg/python/eye/eye/",
                          p_in = "01_data/",
                          p_out = "02_results/",
                          p_fig = "06_fig/",
    ...
    f1_read_files = function() {
      ...
      self$df_wfm_raw <- read_delim(str_c(self$p_in, self$wfm_file), col_names = c("s", "v"), col_types = list(col_double(), col_double()), delim = " ")
      self$df_ref_raw <- read_delim(str_c(self$p_in, self$ref_file), col_names = c("ch", "mk1", "mk2"), col_types = list(col_double(), col_integer(), col_integer()), delim = ",")
    ...
    f1_view_raw_wfm_and_ref = function() {
      # dt <- diff(self$df_wfm_raw$t)[1]
      # private$Fs <- 1 / dt
      # private$os <- private$Fs / self$F
      # message("Sampling rate ", private$Fs)
      # message("os ", private$os)
    ...
    f2_upsample = function(plot = F) {
      ...
      if ((self$Fs / self$F) > 0) {
        message("  F = ", self$F, " Fs = ", self$Fs, " fractional sampling, do interpolation")
        ...
        message("  F = ", self$F, " Fs = ", self$Fs, "up by ", up, "x, new os ", private$os)
        ...
        dfp <- rbind(
          ...
          tibble(s = self$wfm_t[1:(cnt * up)], v = self$wfm_v[1:(cnt * up)], type = "upsampled")
        )
    ...
    f3_detect_pattern = function(plot = F) {
      private$format_message("f3: detect prbs length and update transition tags")
      ref_one_cycle <- self$df_ref_raw$mk1

      c <- ccf(ref_one_cycle, ref_one_cycle, plot = T, lag.max = (length(ref_one_cycle) - 1))
      c_max <- max(c$acf)
      c_max_lag <- c$lag[which(c$acf == c_max)] # it is 0
      high_corr_lags <- c$lag[which(c$acf > 0.2)]
      private$prbs_length <- high_corr_lags[which(high_corr_lags > c_max_lag)][1]
      ...
      ref_one_prbs_with_tags <- set_names(pats[1:private$prbs_length], tags[(private$prbs_length + 1):(private$prbs_length * 2)])
      private$ref_raw_with_tags <- rep(ref_one_prbs_with_tags, ceiling(length(ref_one_cycle) / private$prbs_length))[1:length(ref_one_cycle)]
    ...
    f4_roll_ref = function(plot = F) {
      ...
      c <- ccf(wfm_one_cycle, ref_one_cycle, lag.max = (length(wfm_one_cycle) - 1), plot = F)
      c_max <- max(c$acf)
      c_min <- min(c$acf)
      ...
      if (c_max > abs(c_min)) {
        ...
        i <- c_min_lag
        ref_one_cycle_rolled <- c(tail(rev(ref_one_cycle_invert), i), head(rev(ref_one_cycle_invert), -i))
        message("  inverted ref samples correlate wfm, roll ref ", i, " to align with wfm")
      }

      self$ref_v <- rep(ref_one_cycle_rolled, ceiling(length(self$wfm_v) / length(ref_one_cycle_rolled)))[1:length(self$wfm_v)]
      self$ref_t <- (seq_along(self$ref_v) - 1) * (1 / self$Fs)

      if (plot) {
        dfp <- rbind(
          tibble(`#` = seq_along(wfm_one_cycle), y = wfm_one_cycle, type = "wfm"),
          tibble(`#` = seq_along(wfm_one_cycle), y = ref_one_cycle_rolled, type = "ref")
        )
        p <- ggplot(
          dfp,
    ...
    f5_find_zero_crossing = function(plot = F) {
      ...
      t <- private$zero_crossing(self$ref_t, self$ref_v, thr = 0, find_rising_edge = T, interp = F)
      self$ref_rise_i <- t$i
      self$ref_rise_t <- t$x
      self$ref_rise_v <- t$y
      t <- private$zero_crossing(self$ref_t, self$ref_v, thr = 0, find_rising_edge = F, interp = F)
      self$ref_fall_i <- t$i
      self$ref_fall_t <- t$x
      self$ref_fall_v <- t$y
      message("  find ref zero crossing, seperate rising and falling edges")

      t <- private$zero_crossing_wfm(self$ref_rise_i, self$wfm_t, self$wfm_v, find_rising_edge = T, interp = T)
      self$wfm_rise_i <- t$i
      self$wfm_rise_t <- t$x
      self$wfm_rise_v <- t$y
      t <- private$zero_crossing_wfm(self$ref_fall_i, self$wfm_t, self$wfm_v, find_rising_edge = F, interp = T)
      self$wfm_fall_i <- t$i
      self$wfm_fall_t <- t$x
      self$wfm_fall_v <- t$y
      message("  find wfm zero crossing, seperate rising and falling edges, refer ref crossing edges")
    },
    f6_gen_tie = function(plot = F) {
      ...
      first_zero_crossing_i <- ifelse(is_rising_edge_leading, self$ref_rise_i[1], self$ref_rise_i[1])
      first_zero_cross_wfm_t <- ifelse(is_rising_edge_leading, self$wfm_rise_t[1], self$wfm_fall_t[1])
      message("  determin first wfm zero crossing location and time")

      self$ref_clk_rising_edges_t <- (self$ref_rise_i - first_zero_crossing_i) * 1 / self$Fs + first_zero_cross_wfm_t
      self$ref_clk_falling_edges_t <- (self$ref_fall_i - first_zero_crossing_i) * 1 / self$Fs + first_zero_cross_wfm_t
      ...
      if (is_rising_edge_leading) {
        # self$tie_t <- unlist(mapply(list, self$ref_clk_rising_edges_t, self$ref_clk_falling_edges_t, SIMPLIFY = F))
        # self$tie_in_pS <- unlist(mapply(list, tie_rise, tie_fall, SIMPLIFY = F)) * 1e12
        self$tie_t <- private$interleave(self$ref_clk_rising_edges_t, self$ref_clk_falling_edges_t)
        self$tie_in_pS <- private$interleave(tie_rise, tie_fall) * 1e12
      } else {
        # self$tie_t <- unlist(mapply(list, self$ref_clk_falling_edges_t, self$ref_clk_rising_edges_t, SIMPLIFY = F))
        # self$tie_in_pS <- unlist(mapply(list, tie_fall, tie_rise, SIMPLIFY = F)) * 1e12
        self$tie_t <- private$interleave(self$ref_clk_falling_edges_t, self$ref_clk_rising_edges_t)
      ...
      message("  TIE jitter PkPk (pS) = ", sprintf("%.3f", self$tie_jitter_pkpk_in_pS))

      t_max <- (private$os * length(private$ref_raw_with_tags) * 1 / self$Fs) / 10
      if (plot) {
        dfp <- rbind(
          tibble(x = self$wfm_t, y = self$wfm_v, type = "wfm"),
          tibble(x = self$wfm_rise_t, y = self$wfm_rise_v, type = "zero crossing - rising edge"),
          tibble(x = self$wfm_fall_t, y = self$wfm_fall_v, type = "zero crossing - falling edge")
        ) %>% filter(x <= t_max)
        p <- ggplot(dfp, aes(x, y, color = type, shape = type)) +
          ...
          scale_shape_manual(values = c(wfm = 4, `zero crossing - rising edge` = 4, `zero crossing - falling edge` = 3)) +
          scale_color_brewer(palette = "Dark2") +
          scale_fill_brewer(palette = "Dark2") +
        ...
        ggp <- ggplotly(p)
        print(ggp)
        html_name <- str_c(self$p_fig, "f5_find_zero_crossing.html")
        htmlwidgets::saveWidget(ggp, html_name)
        message("  save to ", html_name)
      }
    },
    f7_gen_eye_db = function() {
      ...
      self$eye_samples_per_Window <- (self$eye_window_length_l + self$eye_window_length_r) * self$eye_samples_per_T

      temp <- min(self$wfm_rise_i[1], self$wfm_fall_i[1]) - (self$eye_window_length_l - 0.5) * self$eye_samples_per_T
      self$eye_start_i <- ifelse(temp > 0, temp, (temp + ceiling(-temp / self$eye_samples_per_T) * self$eye_samples_per_T))

      max_iter <- floor((length(self$wfm_v) - self$eye_start_i + 1) / self$eye_samples_per_Window)
      self$eye_stop_i <- self$eye_start_i + max_iter * self$eye_samples_per_Window - 1

      eye_db <- list()
      eye_db_i <- list()
      self$eye_db_transition_tags <- c()
      for (k in 1:max_iter) {
        slice_index <- self$eye_start_i + (k - 1) * self$eye_samples_per_T + (1:self$eye_samples_per_Window - 1)
        ...
        if (length(valid_tags) > 0) {
          self$eye_db_transition_tags <- c(self$eye_db_transition_tags, valid_tags[1])
        } else {
    ...
    f8_gen_eye_jitter = function(tag = NA) {
      ...
      if (!is.null(self$eye_db_transition_tags) && !is.na(tag)) {
        ...
        stopifnot("  !!! tag not found, expected format: 3T - 1T !!!" = length(tag_index) > 0)
      ...
      for (j in 1:n_col) {
        ...
        b <- y0[length(y0)]
        if ((a < b) && (a * b < 0)) { # rising edge
          t <- private$zero_crossing(x0, y0, thr = thr, find_rising_edge = T, interp = T)
          wi <- t$i
          wx <- t$x
          wy <- t$y
        ...
        if ((a > b) && (a * b < 0)) { # falling edge
          t <- private$zero_crossing(x0, y0, thr = thr, find_rising_edge = F, interp = T)
          wi <- t$i
          wx <- t$x
          wy <- t$y
          jitter_t0 <- c(jitter_t0, wx)
        ...
        b <- y1[length(y1)]
        if ((a < b) && (a * b < 0)) { # rising edge
          t <- private$zero_crossing(x1, y1, thr = thr, find_rising_edge = T, interp = T)
          wi <- t$i
          wx <- t$x
          wy <- t$y
        ...
        if ((a > b) && (a * b < 0)) { # falling edge
          t <- private$zero_crossing(x1, y1, thr = thr, find_rising_edge = F, interp = T)
          wi <- t$i
          wx <- t$x
          wy <- t$y
          jitter_t1 <- c(jitter_t1, wx)
      ...
      if (is.na(tag)) {
        ...
        message("  eye jitter PkPk ", sprintf("%.3f", self$eye_jitter_pkpk_in_pS), "pS")
      } else {
        self$eye_jitter_in_pS_for_tag <- jitter_t0 - crossing_t0
        self$eye_jitter_pkpk_in_pS_for_tag <- max(jitter_t0) - min(jitter_t0)
        message("  eye jitter PkPk ", sprintf("%.3f", self$eye_jitter_pkpk_pS_for_tag), "pS")
      ...
      message("  update eye width ", sprintf("%.3f", self$eye_width_in_pS), "pS")
      message("  update eye datarate ", sprintf("%.3f", self$eye_datarate_Gbps), "Gbps")
    },
    f9_view_sliced_wfm = function(tag = NA, cnt = 4) {
      ...
      if (is.na(tag)) {
        ...
        x_end_min <- min(self$eye_db_i[, ((ncol(self$eye_db_i) - n + 1):ncol(self$eye_db_i))])
        x_end_max <- max(self$eye_db_i[, ((ncol(self$eye_db_i) - n + 1):ncol(self$eye_db_i))])
        t_start <- tibble(`#` = seq(x_start_min, x_start_max), v = self$wfm_v[x_start_min:x_start_max], type = "wfm")
        t_start <- rbind(t_start, tibble(`#` = seq(x_start_min, x_start_max), v = self$ref_v[x_start_min:x_start_max], type = "ref"))
        levels <- c("wfm", "ref")
        for (i in 1:n) {
          t_start <- rbind(t_start, tibble(`#` = self$eye_db_i[, i], v = self$eye_db[, i], type = str_c("#", i)))
          levels <- c(levels, str_c("#", i))
        }
        t_start$type <- factor(t_start$type, levels = levels)
        t_end <- tibble(`#` = seq(x_end_min, x_end_max), v = self$wfm_v[x_end_min:x_end_max], type = "wfm")
        t_end <- rbind(t_end, tibble(`#` = seq(x_end_min, x_end_max), v = self$ref_v[x_end_min:x_end_max], type = "ref"))
        levels <- c("wfm", "ref")
        for (i in n:1) {
          t_end <- rbind(t_end, tibble(`#` = self$eye_db_i[, (ncol(self$eye_db_i) - i + 1)], v = self$eye_db[, (ncol(self$eye_db_i) - i + 1)], type = str_c("#-", abs(-i + 1))))
        ...
        x_end_min <- min(loc_eye_db_i[, ((ncol(loc_eye_db_i) - n + 1):ncol(loc_eye_db_i))])
        x_end_max <- max(loc_eye_db_i[, ((ncol(loc_eye_db_i) - n + 1):ncol(loc_eye_db_i))])
        t_start <- tibble(`#` = seq(x_start_min, x_start_max), v = self$wfm_v[x_start_min:x_start_max], type = "wfm")
        t_start <- rbind(t_start, tibble(`#` = seq(x_start_min, x_start_max), v = self$ref_v[x_start_min:x_start_max], type = "ref"))
        levels <- c("wfm", "ref")
        for (i in 1:n) {
          t_start <- rbind(t_start, tibble(`#` = loc_eye_db_i[, i], v = loc_eye_db[, i], type = str_c("#", i)))
          levels <- c(levels, str_c("#", i))
        }
        t_start$type <- factor(t_start$type, levels = levels)
        t_end <- tibble(`#` = seq(x_end_min, x_end_max), v = self$wfm_v[x_end_min:x_end_max], type = "wfm")
        ...
        levels <- c("wfm", "ref")
        for (i in n:1) {
          t_end <- rbind(t_end, tibble(`#` = loc_eye_db_i[, (ncol(loc_eye_db_i) - i + 1)], v = loc_eye_db[, (ncol(loc_eye_db_i) - i + 1)], type = str_c("#-", abs(-i + 1))))
          levels <- c(levels, str_c("#-", abs(-i + 1)))
        }
        t_end$type <- factor(t_end$type, levels = levels)
      }
    ...
    f9_ggplot_zoomin_wfm_ref_tie = function(max_lines = NULL, color = "steelblue", bins = 50) {
      t_max <- (private$os * length(private$ref_raw_with_tags) * 1 / self$Fs) / 10 * 1e9 # nS
      ...
      p_wfm <- ggplot(data_wfm, aes(x, y, color = type, shape = type)) +
        ...
        scale_shape_manual(values = c(`wfm (zoom in)` = 4, `zero crossing - rising edge` = 4, `zero crossing - falling edge` = 3)) +
        scale_color_brewer(palette = "Dark2") +
        scale_fill_brewer(palette = "Dark2") +
        labs(title = str_c("total wfm length: ", length(self$wfm_v), "points, ", max(self$wfm_t) * 1e9, "nS"), x = "nS", y = "V") +
        theme_bw() +
        theme(legend.title = element_blank(), legend.position = "top")

      # data_ref <- tibble(x = (seq_along(self$ref_v) - 1) * (1 / self$Fs) * 1e12, y = self$ref_v, type = "genie (zoom in)")
      data_ref <- tibble(x = self$ref_t * 1e9, y = self$ref_v, type = "genie (zoom in)")
      p_ref <- ggplot(data_ref %>% filter(x <= t_max), aes(x, y, color = type)) +
        geom_step() +
        scale_x_continuous(limits = c(0, t_max)) +
      ...
      data_tie <- tibble(x = self$tie_t * 1e9, y = self$tie_in_pS, type = "tie (zoom in)")
      p_tie <- ggplot(data_tie %>% filter(x <= t_max), aes(x, y, color = type)) +
        geom_line() +
        geom_point() +
        scale_x_continuous(limits = c(0, t_max)) +
        scale_color_brewer(palette = "Dark2") +
      ...
      if (is.null(max_lines)) {
        data_tie_for_hist <- tibble(x = self$tie_t * 1e9, y = self$tie_in_pS, type = "tie (zoom in)")
      } else {
        tie_index <- which(self$tie_t <= (tail(self$eye_db_i[, max_lines], 1) - 1) * 1 / self$Fs)
        data_tie_for_hist <- tibble(x = self$tie_t[tie_index] * 1e9, y = self$tie_in_pS[tie_index], type = "tie (zoom in)")
      }
      ...
      return(list("p_wfm" = p_wfm, "p_ref" = p_ref, "p_tie" = p_tie, "p_tie_hist" = p_tie_hist))
    },
    f9_ggplot_eye = function(loc_eye_db = NULL, index = NULL, max_lines = 500, color = "steelblue", alpha = 0.2, bins = 50) {
      ...
      for (k in 1:max_lines) {
        p <- p + geom_line(aes(y = .data[[names(t)[k]]]), color = color, alpha = alpha)
      }
      p <- p + labs(
        title = str_c(sprintf("%.3f", self$eye_datarate_Gbps), "Gbps, ", max_lines, "UI"),
        x = "T",
        y = "V"
      ) + theme_bw()
      return(p)
    },
    f9_ggplot_eye_and_wfm = function(loc_eye_db = NULL, index = NULL, max_lines = 500, color = "steelblue", alpha = 0.2, bins = 50) {
      l <- self$f9_ggplot_zoomin_wfm_ref_tie(max_lines, color, bins)

      p_eye <- self$f9_ggplot_eye(loc_eye_db, index, max_lines, color, alpha, bins)
      ...
      p <- l$p_wfm + l$p_ref + l$p_tie + p_eye + l$p_tie_hist + plot_layout(design = design)
      print(p)
    ...
    f9_ggmatplot_eye = function(loc_eye_db = NULL, index = NULL, max_lines = 500, color = "steelblue", alpha = 0.2, bins = 50) {
      # loc_eye_db is the matrix
      if (is.null(loc_eye_db)) loc_eye_db <- self$eye_db
      if (is.null(index)) {
        if (is.null(self$eye_t)) {
    ...
    f9_ggmatplot_eye_and_wfm = function(loc_eye_db = NULL, index = NULL, max_lines = 500, color = "steelblue", alpha = 0.2, bins = 50) {
      l <- self$f9_ggplot_zoomin_wfm_ref_tie(max_lines, color, bins)

      p_eye <- self$f9_ggmatplot_eye(loc_eye_db, index, max_lines, color, alpha, bins)
    ...
    f9_matplot_eye = function(loc_eye_db = NULL, index = NULL, max_lines = NULL, color = "steelblue", alpha = 0.2, bins = 50) {
      # loc_eye_db is the matrix
      if (is.null(loc_eye_db)) loc_eye_db <- self$eye_db
      if (is.null(index)) {
        if (is.null(self$eye_t)) {
      ...
      matplot(
        ...
        xlab = "T",
        ylab = "V",
        main = str_c(sprintf("%.3f", self$eye_datarate_Gbps), "Gbps, ", max_lines, "UI")
      )
    },
    f9_matplot_eye_and_wfm = function(loc_eye_db = NULL, index = NULL, max_lines = NULL, color = "steelblue", alpha = 0.2, bins = 50) {
      l <- self$f9_ggplot_zoomin_wfm_ref_tie(max_lines, color, bins)

      # png(str_c(self$p_fig, "f9_matplot_eye.png"), width = 12, height = 6, units = "in", res = 300)

      layout(matrix(c(2, 2, 1, 3, 3, 1, 4, 4, 5), nrow = 3, ncol = 3, byrow = T), heights = c(2, 2, 2), widths = c(2, 2, 2))
