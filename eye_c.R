...
Eye <- R6Class(
  ...
  public = list(
    ...
    f6_gen_tie = function(plot = F) {
      ...
      first_zero_crossing_i <- ifelse(is_rising_edge_leading, self$ref_rise_i[1], self$ref_fall_i[1])
    ...
    f9_ggplot_zoomin_wfm_ref_tie = function(max_lines = NULL, color = "steelblue", bins = 50) {
      ...
      p_ref <- ggplot(data_ref %>% dplyr::filter(x <= t_max), aes(x, y, color = type)) +
        geom_step() +
        scale_x_continuous(limits = c(0, t_max)) +
      ...
      p_tie <- ggplot(data_tie %>% dplyr::filter(x <= t_max), aes(x, y, color = type)) +
        geom_line() +
        geom_point() +
        scale_x_continuous(limits = c(0, t_max)) +
        scale_color_brewer(palette = "Dark2") +
